...
public class ProductDaoImpl implements ProductDao {
    ...
    public List<Product> findActiveRecommendations(String keyword, UUID excludeProductId, int limit) {
        ...
        String jpql = "SELECT p FROM Product p WHERE p.productId != :excludeId AND p.status = '1' AND p.isDeleted = false " +
                "AND (LOWER(p.productName) LIKE LOWER(:keyword) OR LOWER(p.productDesc) LIKE LOWER(:keyword))";

        return em.createQuery(jpql, Product.class)
                .setParameter("excludeId", excludeProductId)
                .setParameter("keyword", "%" + keyword + "%") // 包含 keyword 的
                .setMaxResults(limit)
                .getResultList();
        // Django original query used split keywords, which is more complex in JPQL.
    }
    ...
    public PaginatedResult<Product> findActiveByCriteria(ProductSearchCriteria criteria) {
        // 对应 SearchView
        StringBuilder jpql = new StringBuilder("SELECT p FROM Product p WHERE p.status = '1' AND p.isDeleted = false");
    ...
    public PaginatedResult<Product> findAdminByKeyword(String keyword, int page, int pageSize) {
        // 对应 get_product_view
        // 只看 is_deleted = false，不管 status
        StringBuilder jpql = new StringBuilder("SELECT p FROM Product p WHERE p.isDeleted = false");
        StringBuilder countJpql = new StringBuilder("SELECT COUNT(p) FROM Product p WHERE p.isDeleted = false");
        Map<String, Object> parameters = new HashMap<>();

        if (keyword != null && !keyword.trim().isEmpty()) {
            String likeKeyword = "%" + keyword.toLowerCase() + "%";
            jpql.append(" AND (LOWER(p.productName) LIKE LOWER(:keyword) OR LOWER(p.productId) LIKE LOWER(:keyword))");
            countJpql.append(" AND (LOWER(p.productName) LIKE LOWER(:keyword) OR LOWER(p.productId) LIKE LOWER(:keyword))");
            parameters.put("keyword", likeKeyword);
        }

        jpql.append(" ORDER BY p.updatedTime DESC"); // 默认排序
    ...
    private void buildSearchWhereClause(StringBuilder jpql, StringBuilder countJpql, Map<String, Object> parameters,
        ...
        if (keyword != null && !keyword.trim().isEmpty()) {
            String likeKeyword = "%" + keyword.toLowerCase() + "%";
            jpql.append(" AND (LOWER(p.productName) LIKE LOWER(:keyword) OR LOWER(p.productDesc) LIKE LOWER(:keyword))");
            countJpql.append(" AND (LOWER(p.productName) LIKE LOWER(:keyword) OR LOWER(p.productDesc) LIKE LOWER(:keyword))");
            parameters.put("keyword", likeKeyword);
        }
